...
SHIELD = """<svg width="48" height="54" viewBox="0 0 80 90" fill="none" xmlns="http://www.w3.org/2000/svg">
...
SHIELD_BIG = """<svg width="90" height="100" viewBox="0 0 80 90" fill="none" xmlns="http://www.w3.org/2000/svg">
...
def show_navbar():
    ...
    with col_logo:
        st.markdown(f'<div style="display:flex;align-items:center;gap:12px;padding:12px 0;">{SHIELD}<div><div style="font-family:Syne,sans-serif;font-size:20px;font-weight:800;color:#f5a623;">MEDICLASSIFY</div><div style="font-size:10px;color:#9b98cc;">Diagnose Faster. Treat Better.</div></div></div>', unsafe_allow_html=True)
    with col_links:
...
def show_about():
    ...
    st.markdown('<div class="glass-card"><div class="sec-title">Project overview</div><p style="font-size:14px;color:rgba(240,240,255,0.8);line-height:1.8;"><strong style="color:#f5a623">MediClassify</strong> is a Placement Mini Project — a Medical Report Classification System built using Python and Machine Learning. It automatically reads a medical report and categorizes it into Radiology, Lab Reports, Cardiology, and Clinical Notes.</p></div>', unsafe_allow_html=True)
    ...
    for col,(icon,name,desc) in zip(st.columns(4),techs):
        with col: st.markdown(f'<div class="feature-card" style="margin-bottom:16px;"><div class="feature-icon">{icon}</div><div class="feature-title">{name}</div><div class="feature-desc">{desc}</div></div>', unsafe_allow_html=True)
    st.markdown('<div class="glass-card"><div class="sec-title">How it works</div>', unsafe_allow_html=True)
    for i,(icon,label) in enumerate([("📄","Medical report entered"),("🔤","Text cleaned with NLP"),("📐","TF-IDF vectorization"),("🤖","Naive Bayes predicts"),("✅","Output with confidence")]):
        ...
        if i<4: st.markdown('<div style="text-align:center;color:#f5a623;font-size:22px;margin:-2px 0">↓</div>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
...
def show_contact():
    ...
    with left:
        ...
        if st.button("📨  Send Message"):
            ...
            else: st.success(f"✅ Thank you {c_name}! We will reply to {c_email} soon.")
        st.markdown('</div>', unsafe_allow_html=True)
    with right:
...
def show_dashboard():
    show_navbar()
    st.markdown(f'<div style="display:flex;align-items:center;justify-content:space-between;background:#252360;border:1px solid #3d3a8a;border-radius:20px;padding:22px 32px;margin-bottom:24px;"><div style="display:flex;align-items:center;gap:16px;">{SHIELD}<div><div style="font-family:Syne,sans-serif;font-size:24px;font-weight:800;color:#f5a623;">MEDICLASSIFY</div><div style="font-size:12px;color:#9b98cc;">Diagnose Faster. Treat Better.</div></div></div><div style="background:rgba(245,166,35,0.1);border:1px solid rgba(245,166,35,0.35);color:#f5a623;padding:8px 20px;border-radius:20px;font-size:13px;"><span class="pulse"></span>AI Active</div></div>', unsafe_allow_html=True)
    total=sum(st.session_state.counts.values())
    c1,c2,c3,c4=st.columns(4)
    with c1: st.markdown(f'<div class="stat-box"><div class="stat-num">{total}</div><div class="stat-lbl">Total</div></div>', unsafe_allow_html=True)
    ...
    with c3: st.markdown(f'<div class="stat-box"><div class="stat-num" style="color:#00d4b4">{st.session_state.counts["Lab Report"]}</div><div class="stat-lbl">Lab Report</div></div>', unsafe_allow_html=True)
    ...
    with left:
        ...
        if input_type in ["🖼️ Upload Image","📝 + 🖼️ Both"]:
            ...
            if uploaded_img and uploaded_img.type in ["image/jpeg","image/png","image/jpg"]:
        ...
        st.markdown('</div>', unsafe_allow_html=True)
        if classify_btn:
            if not p_report.strip(): st.error("⚠️ Please type the report text to classify!")
            else:
                category,confidence=classify_report(model,p_report)
                name=p_name.strip() or "Unknown patient"
                meta=" · ".join(filter(None,[f"Age {p_age}" if p_age else "",p_gender,f"Dr: {p_doc}" if p_doc else ""]))
                color=cat_color[category]; css=cat_css[category]
                img_badge=f'<span style="background:rgba(245,166,35,0.15);color:#f5a623;border:1px solid rgba(245,166,35,0.3);padding:2px 10px;border-radius:10px;font-size:10px;margin-left:8px;">+ Image</span>' if uploaded_img else ""
                ...
                st.markdown(f'<div class="{css}"><div style="font-size:12px;color:{color};margin-bottom:4px;">Classification for {name}{" — "+meta if meta else ""} {img_badge}</div><div class="result-cat" style="color:{color}">{category}</div><div class="result-info" style="color:{color}">{confidence}% confidence | Naive Bayes ML</div>{rx_html}</div>', unsafe_allow_html=True)
                st.session_state.counts[category]+=1
                now=datetime.datetime.now()
                st.session_state.history.insert(0,{"name":name,"age":p_age,"gender":p_gender,"doc":p_doc,"date":str(p_date),"time":now.strftime("%I:%M %p"),"report":p_report[:65]+("..." if len(p_report)>65 else ""),"rx":(p_rx[:50]+"...") if len(p_rx)>50 else p_rx,"cat":category,"conf":confidence,"has_img":uploaded_img is not None})
                st.rerun()

    with right:
        with st.expander("📊 Project flow",expanded=False):
            for i,(icon,label) in enumerate([("📄","Medical report (text/image)"),("🔤","Clean text — NLP"),("🤖","Naive Bayes ML model"),("🎯","Predict category"),("✅","Output result")]):
                st.markdown(f'<div class="flow-step"><span style="font-size:18px">{icon}</span> {label}</div>', unsafe_allow_html=True)
                if i<4: st.markdown('<div style="text-align:center;color:#f5a623;font-size:20px;margin:-3px 0">↓</div>', unsafe_allow_html=True)
        ...
        if not st.session_state.history:
            ...
            for h in st.session_state.history[:12]:
                meta=" · ".join(filter(None,[f"Age {h['age']}" if h['age'] else "",h['gender'],f"Dr: {h['doc']}" if h['doc'] else ""]))
                ...
                st.markdown(f'<div class="hist-card"><div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:5px"><span class="hist-name">{h["name"]}{img_tag}</span><span class="badge {bc}">{h["cat"]}</span></div>{f\'<div class="hist-meta">{meta}</div>\' if meta else ""}<div class="hist-rep">{h["report"]}</div>{rx_html}<div class="hist-time">{h["date"]} at {h["time"]} | {h["conf"]}% confidence</div></div>', unsafe_allow_html=True)
